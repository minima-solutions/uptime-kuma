<template>
    ...
    <div class="mb-3">
        <label for="webhook-http-method" class="form-label">{{
            $t("HTTP Method")
        }}</label>
        <select
        ...
        <div class="form-text">
            {{
                $parent.notification.httpMethod === "get"
                    ? $t("webhookGetMethodDesc")
                    : $t("webhookPostMethodDesc")
            }}
        </div>
    </div>
...
export default {
    ...
    "Authorization": "Authorization Token"
        ...
        customBodyPlaceholder() {
            return this.$t("Example:", [
                `{
    "Title": "MINIMA Status Alert{% if monitorJSON %} - {{ monitorJSON['name'] }}{% endif %}",
    "Body": "{{ msg }}"
}`,
            ]);
        },
