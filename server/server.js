/*
 * MINIMA Status Server
 * node "server/server.js"
 * DO NOT require("./server") in other modules, it likely creates circular dependency!
 */
console.log("Welcome to MINIMA Status");
...
// Exit MINIMA Status immediately if the Node.js version is banned
...
log.info("server", "MINIMA Status Version:", checkVersion.version);
...
(async () => {
    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if (
                    ...
                    throw new Error(
                        "MINIMA Status has been initialized. If you want to run setup again, please delete the database."
                    );
                }
        ...
        socket.on(
            "deleteMonitor",
            async (monitorID, deleteChildren, callback) => {
                try {
                    ...
                    if (monitor && monitor.type === "group") {
                        if (deleteChildren) {
                            log.info(
                                "manage",
                                `Delete Group and Children: ${monitorID} User ID: ${socket.userID}`
                            );
                        } else {
                            log.info(
                                "manage",
                                `Delete Group (unlink children): ${monitorID} User ID: ${socket.userID}`
                            );
                        }
                    } else {
                        log.info(
                        ...
                        if (deleteChildren) {
                            ...
                            if (children && children.length > 0) {
                                for (const child of children) {
                                    await Monitor.deleteMonitorRecursively(
                                        child.id,
                                        socket.userID
                                    );
                                    await server.sendDeleteMonitorFromList(
                                        socket,
                                        child.id
                                    );
                                }
                            ...
                            if (children && children.length > 0) {
                                for (const child of children) {
                                    await server.sendUpdateMonitorIntoList(
                                        socket,
                                        child.id
                                    );
                                }
                    ...
                    if (monitor && monitor.type === "group") {
                        if (deleteChildren) {
                            log.info(
                                "DB",
                                `Delete Monitor completed (group and children deleted) in: ${
                                    endTime - startTime
                                } ms`
                            );
                        } else {
                            log.info(
                                "DB",
                                `Delete Monitor completed (group deleted, children unlinked) in: ${
                                    endTime - startTime
                                } ms`
                            );
                        }
...
async function initDatabase(testMode = false) {
    ...
    // If there is no record in user table, it is a new MINIMA Status instance, need to setup
